...
async def load_data(items: List[NasaApodInput], db: AsyncSession = Depends(get_async_db)):
    ...
    for item in items:
        ...
        item_to_save = schemas.DataSave(
            title=item.title, url=item.url, date=item.date, embedding=embedding
        )
...
async def delete_item(
    item_to_delete: schemas.ItemToDelete, db: AsyncSession = Depends(get_async_db)
):
    """
