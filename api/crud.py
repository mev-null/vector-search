...
async def create_item(db: AsyncSession, item: schemas.DataSave) -> models.Apod:
    ...
    db_item = models.Apod(title=item.title, url=item.url, date=item.date, embedding=item.embedding)
    db.add(db_item)
    await db.commit()
...
async def search_vector(
    db: AsyncSession, query_embedding: List[float], limit: int
) -> List[models.Apod]:
    """
    ...
    stmt = (
        select(models.Apod)
        .order_by(models.Apod.embedding.l2_distance(query_embedding))
        .limit(limit)
    )
...
async def delete_item_by_details(
    db: AsyncSession, item_details: schemas.ItemToDelete
) -> Optional[models.Apod]:
    """
